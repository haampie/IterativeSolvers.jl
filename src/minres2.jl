export minres2_iterable, minres2

import Base.LinAlg: BLAS.axpy!, givensAlgorithm
import Base: start, next, done

type Minres2Iterable{matT, vecT, realT}
    A::matT
    x::vecT
...
minres2_iterable(A, b; kwargs...) = minres2_iterable!(zeros(b), A, b; initially_zero = true, kwargs...)

function minres2_iterable!(x, A, b; initially_zero = false, tol = sqrt(eps(real(eltype(b)))), maxiter = size(A, 1))
    T = real(eltype(b))
...
converged(m::Minres2Iterable) = m.resnorm ≤ m.tolerance

start(::Minres2Iterable) = 1

done(m::Minres2Iterable, iteration::Int) = iteration > m.maxiter || converged(m)

function next(m::Minres2Iterable, iteration::Int)
...
function minres2!(x, A, b; kwargs...)
    iterable = minres2_iterable!(x, A, b; kwargs...)
...
minres2(A, b; kwargs...) = minres2!(zeros(b), A, b; initially_zero = true, kwargs...)